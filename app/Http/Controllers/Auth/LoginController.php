...
class LoginController extends Controller
{
    ...
    public function loginPage(){
        ...
        return view('login', compact('recaptchaSetting','socialLogin'));
    }
        ...
        if($user){
            if($user->status==1){
                if(Hash::check($request->password,$user->password)){
                    ...
                    $notification = "Credentials does not exist";
                    $notification=array('messege'=>$notification,'alert-type'=>'error');
                    return redirect()->back()->with($notification);
                }

            }else{
                $notification = "Disabled Account";
                $notification=array('messege'=>$notification,'alert-type'=>'error');
                return redirect()->back()->with($notification);
            }
        }else{
            $notification = "Email does not exist";
            $notification=array('messege'=>$notification,'alert-type'=>'error');
            return redirect()->back()->with($notification);
        }
    }
    ...
    public function forgetPage(){
        $recaptchaSetting = GoogleRecaptcha::first();
        return view('forget_password', compact('recaptchaSetting'));
    }
